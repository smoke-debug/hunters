...
# Optional role IDs for auto-DM guides when roles are given.
...
async def post_claim_to_channel(guild: discord.Guild, claim: dict, *, pending: bool = False) -> None:
...
async def delete_claim_message(guild: discord.Guild, claim: dict, *, pending: bool = False) -> None:
    cfg = get_config(guild.id)
    channel_id = cfg.get("pending_claim_channel_id") if pending else cfg.get("claim_log_channel_id")
    key = "pending_message_id" if pending else "message_id"
...
async def apply_autoroles(guild: discord.Guild, member: discord.Member) -> None:
    ...
    for rule in sorted(cfg.get("autoroles", []), key=lambda x: int(x.get("claims_required", 0))):
        ...
        if role and total >= req and role not in member.roles:
            try:
                await member.add_roles(role, reason=f"Reached {req} approved vanity claims")
            except Exception:
                pass
...
def stats_embed(guild: discord.Guild, user_id: int) -> discord.Embed:
    ...
    approved_like = [c for c in claims if c.get("status") in {"approved", "sold", "paid"}]
    e = make_embed("📊 Hunter Stats", f"Stats for <@{user_id}>", PURPLE)
    e.add_field(name="Claims", value=f"Approved/Sold/Paid: `{len(approved_like)}`\nTotal Submitted: `{len(claims)}`", inline=True)
    e.add_field(name="Payment", value=f"`{get_payment_method(guild.id, user_id) or 'Not set'}`", inline=True)
    e.add_field(name="Total Value", value=f"`{money(sum(float(c.get('value') or 0) for c in approved_like))}`", inline=True)
    lines = [
        f"`discord.gg/{clean_code(c.get('code'))}` • `{c.get('status', 'pending')}` • ID `{c.get('id')}`"
        for c in claims[:15]
    ]
...
def hunter_guide_embed() -> discord.Embed:
    ...
    e.description = (
        ...
        "3. Try as many vanities as you can until the server gets rate limited.\n"
        "4. Go to **Server Settings → Custom URL** to try the words.\n"
        "5. If rate limited, wait `15–30 minutes` before trying again.\n"
        "6. Repeat until you successfully claim a word from the lists.\n\n"
        "**What to do if you claim a word:**\n"
        "1. Use the Hunter Panel or `/hunter_panel` and click **Log Claim**.\n"
        "2. Enter the vanity you claimed.\n"
        "3. Wait for a manager/owner to review it.\n"
        "4. If approved, you will get a DM and wait for a new boosted server.\n"
        "5. If denied, keep trying words until you get one approved for sales.\n\n"
        "⚠️ Claims are blocked unless the vanity is currently valid. Fake or spammed claims can get you removed."
    )
    return e
...
def hunter_panel_embed() -> discord.Embed:
    ...
    e.description = (
        "Use this panel to log successful vanity pulls and view stats.\n\n"
...
def manager_panel_embed() -> discord.Embed:
    ...
    e.description = (
        "Control pending claims, posted claims, user notes, payment checks, buyer/status info, settings, and autoroles.\n\n"
...
class LogClaimModal(discord.ui.Modal, title="Log Vanity Claim"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        payment = get_payment_method(interaction.guild.id, interaction.user.id)
        if not payment:
            return await interaction.response.send_message(
                "❌ You must set your payment method before claiming.\nUse `/set_payment` first. No payment set = no payouts.",
                ephemeral=True,
            )
        ...
        await post_claim_to_channel(interaction.guild, claim, pending=True)
        await interaction.response.send_message(
            f"📩 Submitted `discord.gg/{code}` for approval. A manager will review it.",
            ephemeral=True,
        )
...
class ManagerEditClaimModal(discord.ui.Modal, title="Manager Edit Claim"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await post_claim_to_channel(interaction.guild, claim, pending=(claim.get("status") == "pending"))
        if claim.get("status") in {"approved", "sold", "paid"}:
            await post_claim_to_channel(interaction.guild, claim, pending=False)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if status in {"approved", "sold", "paid"}:
            ...
            if old_status != status:
                await safe_dm(user, f"✅ Your claim `discord.gg/{clean_code(claim.get('code'))}` was approved. An owner/manager will follow up with the next steps.")
            if member:
                await apply_autoroles(interaction.guild, member)
...
class AddUserNoteModal(discord.ui.Modal, title="Add User Note"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        notes.setdefault(str(uid), []).append({
            "id": make_id(),
...
class ViewUserNotesModal(discord.ui.Modal, title="View User Notes"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not items:
            ...
            e.add_field(name="Recent Notes", value=truncate_field("\n\n".join(lines), 4000), inline=False)
...
class ManagerConfigModal(discord.ui.Modal, title="Manager Config"):
    claim_log_channel_id = discord.ui.TextInput(label="Approved claim log channel ID/mention", required=False, max_length=40)
    pending_claim_channel_id = discord.ui.TextInput(label="Pending claim review channel ID/mention", required=False, max_length=40)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        approved_id = parse_snowflake(str(self.claim_log_channel_id))
        pending_id = parse_snowflake(str(self.pending_claim_channel_id))
        if approved_id:
            cfg["claim_log_channel_id"] = approved_id
        if pending_id:
            cfg["pending_claim_channel_id"] = pending_id
...
class AddAutoroleModal(discord.ui.Modal, title="Add Claim Autorole"):
    ...
    claims_required = discord.ui.TextInput(label="Approved claims required", placeholder="10", max_length=10)
...
# =========================================================
# CHECKER EMBEDS / FILES
# =========================================================
def copyable_codes_text(invalids: List[str]) -> str:
    return ", ".join(dict.fromkeys(clean_code(x) for x in invalids if clean_code(x)))

def copyable_links_text(invalids: List[str]) -> str:
    return "\n".join(f"discord.gg/{clean_code(x)}" for x in dict.fromkeys(invalids) if clean_code(x))

def make_invalid_text_file(list_name: str, length: int, invalids: List[str]) -> discord.File:
    safe_name = re.sub(r"[^a-zA-Z0-9_-]", "_", list_name)[:40] or "vanities"
    path = DATA_DIR / "exports" / f"{safe_name}_{length}_letter_invalids.txt"
    lines = [
        f"{length} lettered vanities",
        f"List: {list_name}",
        f"Updated timestamp: {now_ts()}",
        "",
        "COPYABLE WORDS:",
        copyable_codes_text(invalids),
        "",
        "COPYABLE LINKS:",
        copyable_links_text(invalids),
        "",
    ]
...
def list_embed_by_length(list_name: str, length: int, invalids: List[str], became_valid_for_length: List[str], valid_to_invalid_for_length: List[str], processed: int, errors: int) -> discord.Embed:
    ...
    e.description = (
        ...
        "• Use the preview below for quick copying."
    )
    if invalids:
        copy_text = copyable_codes_text(invalids)
        if len(copy_text) <= 950:
            e.add_field(name="Copyable Words", value=f"```txt\n{copy_text}\n```", inline=False)
        else:
            e.add_field(name="Copyable Words Preview", value=f"```txt\n{copyable_codes_text(invalids[:35])}\n```\nFull list is attached.", inline=False)
        shown = [f"`discord.gg/{c}`" for c in invalids[:25]]
        if len(invalids) > 25:
            shown.append(f"...and `{len(invalids)-25}` more in attached file")
        e.add_field(name="Invalid Links Preview", value="\n".join(shown), inline=False)
    else:
...
def change_log_embed(list_name: str, became_valid: List[str], valid_to_invalid: List[str], processed: int, errors: int) -> discord.Embed:
    ...
    e.add_field(name="✅ Was Invalid, Now Valid", value="\n".join(f"`discord.gg/{c}`" for c in became_valid[:35]) or "None this update.", inline=False)
    e.add_field(name="🔥 Was Valid, Now Invalid", value="\n".join(f"`discord.gg/{c}`" for c in valid_to_invalid[:35]) or "None this update.", inline=False)
    return e
...
async def run_two_server_check(guild: discord.Guild, list_name: str, setup: dict) -> dict:
    ...
    for i, length in enumerate(sorted(grouped.keys())):
        ...
        file = make_invalid_text_file(list_name, length, invalids) if invalids else None
        if file:
            msg = await invalid_channel.send(content=content if i == 0 else None, embed=e, file=file, allowed_mentions=allowed)
        else:
    ...
    if valid_channel:
        summary = make_embed("✅ Vanity Check Valid Summary", color=GREEN)
        summary.description = (
            f"**List:** `{list_name}`\n"
        ...
        if old_valid_id:
            try:
                ...
                pass
        msg = await valid_channel.send(embed=summary)
    ...
    if change_log_channel and (became_valid or valid_to_invalid or setup.get("always_send_change_log", True)):
        msg = await change_log_channel.send(embed=change_log_embed(list_name, became_valid, valid_to_invalid, len(codes), errors))
    ...
    state[s_key] = {"invalid": sorted(current_invalid), "valid": sorted(current_valid), "last_run": now_ts()}
...
@bot.tree.command(name="hunter_info", description="Show the hunter guide.")
async def hunter_info(interaction: discord.Interaction):
    await interaction.response.send_message(embed=hunter_guide_embed(), ephemeral=True)

@bot.tree.command(name="manager_info", description="Show the manager guide.")
...
async def help_cmd(interaction: discord.Interaction):
    ...
    e.description = (
        "**Main workflow:**\n"
        "• Hunters use `/set_payment` first.\n"
        "• Hunters use `/hunter_panel` to submit claims.\n"
        "• Claims stay pending until managers approve/deny them.\n"
        "• Managers use `/manager_panel` to control claims, notes, buyers, roles, and settings.\n\n"
        "**Checker workflow:**\n"
        "• Add lists with `/checker_add_list`.\n"
        "• Set two-server posting with `/two_server_list_setup`.\n"
        "• Run now with `/checker_run_now`.\n"
        "• View changes with `/checker_change_logs`."
    )
    await interaction.response.send_message(embed=e, ephemeral=True)
...
@app_commands.describe(method="Example: LTC, CashApp, Nitro, Robux, Decos, etc.")
async def set_payment(interaction: discord.Interaction, method: str):
    ...
    await interaction.response.send_message(f"✅ Payment method saved: `{method}`\nYou can now submit claims.", ephemeral=True)
...
async def delete_payment(interaction: discord.Interaction):
    ...
    save_payments(interaction.guild.id, payments)
    await interaction.response.send_message("🗑️ Payment method deleted. You cannot submit new claims until you set it again." if existed else "No payment method was set.", ephemeral=True)

@bot.tree.command(name="view_payments", description="Manager only: view all hunter payment preferences.")
async def view_payments(interaction: discord.Interaction):
    if not await require_manager(interaction): return
    payments = get_payments(interaction.guild.id)
    e = make_embed("💰 Hunter Payment Preferences", color=GOLD)
    if not payments:
        e.description = "No payment preferences set yet."
    else:
        lines = []
        for uid, data in payments.items():
            lines.append(f"<@{uid}> → `{data.get('method', 'Not set')}` • updated <t:{int(data.get('updated_ts', 0))}:R>")
        e.description = truncate_field("\n".join(lines), 4000)
    await interaction.response.send_message(embed=e, ephemeral=True)

# =========================================================
# SLASH COMMANDS: STATS / MANAGER ACCESS
# =========================================================
...
@bot.tree.command(name="checker_lists", description="Show saved checker lists.")
@app_commands.default_permissions(manage_guild=True)
async def checker_lists(interaction: discord.Interaction):
    if not await require_manager(interaction): return
    lists = get_lists(interaction.guild.id)
    e = make_embed("📋 Saved Checker Lists", color=BLUE)
    e.description = "\n".join(f"`{name}` — `{len(codes)}` words" for name, codes in lists.items()) or "No lists saved yet."
    await interaction.response.send_message(embed=e, ephemeral=True)
...
async def checker_setups(interaction: discord.Interaction):
    ...
    if not checkers:
        ...
        for name, c in checkers.items():
            change_channel = f"<#{c.get('change_log_channel_id')}>" if c.get("change_log_channel_id") else "`not set`"
            lines.append(
                f"`{name}` → invalid <#{c['hunters_invalid_channel_id']}> • changes {change_channel} • every `{c['interval_minutes']}m` • next <t:{int(c.get('next_run', 0))}:R>"
            )
        e.description = "\n".join(lines)
    await interaction.response.send_message(embed=e, ephemeral=True)
...
async def on_member_update(before: discord.Member, after: discord.Member):
    ...
    if HUNTER_ROLE_ID and HUNTER_ROLE_ID in after_roles and HUNTER_ROLE_ID not in before_roles:
        await safe_dm(after, (
            "🏹 You were given the Hunter role.\n\n"
            "Start by running `/set_payment` in the server. You cannot submit claims until payment is set.\n"
            "Then use `/hunter_panel` and read the Info button."
        ))

    if MANAGER_ROLE_ID and MANAGER_ROLE_ID in after_roles and MANAGER_ROLE_ID not in before_roles:
        await safe_dm(after, (
            "🛠️ You were given the Manager role.\n\n"
            "Use `/manager_panel` and read the Manager Guide. Managers review claims, keep logs clean, and help find buyers."
        ))
...
async def on_ready():
    ...
    print("RUNNING FINAL PRODUCTION VANITY BOT PACKAGE")
